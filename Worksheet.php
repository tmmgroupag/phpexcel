...
class Worksheet implements IComparable
{
   ...
   /**
    ...
    {
        $CharCount = Shared\StringClass::CountCharacters($pValue);
        if ($CharCount == 0) {
            throw new Exception('Sheet code name cannot be empty.');
        }
        // Some of the printable ASCII characters are invalid:  * : / \ ? [ ] and  first and last characters cannot be a "'"
        if ((str_replace(self::$invalidCharacters, '', $pValue) !== $pValue) ||
            (Shared\StringClass::Substring($pValue, -1, 1)=='\'') ||
            (Shared\StringClass::Substring($pValue, 0, 1)=='\'')) {
   ...
   /**
    ...
    private static function checkSheetTitle($pValue)
    {
        ...
        // Maximum 31 characters allowed for sheet title
        if (Shared\StringClass::CountCharacters($pValue) > 31) {
    ...
    public function setTitle($pValue = 'Worksheet', $updateFormulaCellReferences = true)
    {
        ...
        if ($this->parent) {
            ...
            if ($this->parent->sheetNameExists($pValue)) {
                ...
                if (Shared\StringClass::CountCharacters($pValue) > 29) {
                    $pValue = Shared\StringClass::Substring($pValue, 0, 29);
                }
                $i = 1;
                while ($this->parent->sheetNameExists($pValue . ' ' . $i)) {
                    ++$i;
                    if ($i == 10) {
                        if (Shared\StringClass::CountCharacters($pValue) > 28) {
                            $pValue = Shared\StringClass::Substring($pValue, 0, 28);
                        }
                    } elseif ($i == 100) {
                        if (Shared\StringClass::CountCharacters($pValue) > 27) {
                            $pValue = Shared\StringClass::Substring($pValue, 0, 27);
                        }
                    }
                }
