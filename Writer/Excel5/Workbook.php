...
/**
 * \PHPExcel\Writer\Excel5\Workbook
 *
...
class Workbook extends BIFFwriter
{
     ...
     * Fonts writers
     *
     * @var \PHPExcel\Writer\Excel5\Font[]
     */
    ...
    private function calcSheetOffsets()
    {
        ...
        foreach ($this->phpExcel->getWorksheetIterator() as $sheet) {
            $offset += $boundsheet_length + strlen(\PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
        }
    ...
    private function writeNames()
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                $printArea[0] = \PHPExcel\Cell::coordinateFromString($printArea[0]);
                $printArea[1] = \PHPExcel\Cell::coordinateFromString($printArea[1]);

                $print_rowmin = $printArea[0][1] - 1;
                $print_rowmax = $printArea[1][1] - 1;
                $print_colmin = \PHPExcel\Cell::columnIndexFromString($printArea[0][0]) - 1;
                $print_colmax = \PHPExcel\Cell::columnIndexFromString($printArea[1][0]) - 1;
    ...
    private function writeAllDefinedNamesBiff8()
    {
        ...
        if (count($this->phpExcel->getNamedRanges()) > 0) {
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                $range = \PHPExcel\Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
    ...
    private function writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
    {
        ...
        $nlen = \PHPExcel\Shared\StringClass::CountCharacters($name);

        // name with stripped length field
        $name = substr(\PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($name), 2);
    ...
    private function writeBoundSheet($sheet, $offset)
    {
        ...
        $data .= \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeShort($sheetname);
    ...
    private function writeNumberFormat($format, $ifmt)
    {
        ...
        $numberFormatString = \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($format);
