                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    if (isset($xmlStrings) && isset($xmlStrings->si)) {
                        foreach ($xmlStrings->si as $val) {
                            if (isset($val->t)) {
                                $sharedStrings[] = \PHPExcel\Shared\StringClass::ControlCharacterOOXML2PHP((string) $val->t);
                            } elseif (isset($val->r)) {
...
//        }
    ...
    private function parseRichText($is = null)
    {
        ...
        if (isset($is->t)) {
            $value->createText(\PHPExcel\Shared\StringClass::ControlCharacterOOXML2PHP((string) $is->t));
        } else {
            if (is_object($is->r)) {
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        $objText = $value->createText(\PHPExcel\Shared\StringClass::ControlCharacterOOXML2PHP((string) $run->t));

                    } else {
                        $objText = $value->createTextRun(\PHPExcel\Shared\StringClass::ControlCharacterOOXML2PHP((string) $run->t));
