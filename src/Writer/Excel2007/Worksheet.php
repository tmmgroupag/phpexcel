...
//      }
    ...
    private function writeSheetFormatPr(\PHPExcel\Shared\XMLWriter $objWriter = null, \PHPExcel\Worksheet $pSheet = null)
    {
        ...
        if ($pSheet->getDefaultRowDimension()->getRowHeight() >= 0) {
            ...
            $objWriter->writeAttribute('defaultRowHeight', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getDefaultRowDimension()->getRowHeight()));
        } else {
        ...
        if ($pSheet->getDefaultColumnDimension()->getWidth() >= 0) {
            $objWriter->writeAttribute('defaultColWidth', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getDefaultColumnDimension()->getWidth()));
        }
        ...
        if (count($pSheet->getColumnDimensions()) > 0) {
            ...
            foreach ($pSheet->getColumnDimensions() as $colDimension) {
                ...
                if ($colDimension->getWidth() < 0) {
                    ...
                    // Width set
                    $objWriter->writeAttribute('width', \PHPExcel\Shared\StringClass::FormatNumber($colDimension->getWidth()));
                }
    ...
    private function writePageMargins(\PHPExcel\Shared\XMLWriter $objWriter = null, \PHPExcel\Worksheet $pSheet = null)
    {
        ...
        $objWriter->writeAttribute('left', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getPageMargins()->getLeft()));
        $objWriter->writeAttribute('right', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getPageMargins()->getRight()));
        $objWriter->writeAttribute('top', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getPageMargins()->getTop()));
        $objWriter->writeAttribute('bottom', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getPageMargins()->getBottom()));
        $objWriter->writeAttribute('header', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getPageMargins()->getHeader()));
        $objWriter->writeAttribute('footer', \PHPExcel\Shared\StringClass::FormatNumber($pSheet->getPageMargins()->getFooter()));
        $objWriter->endElement();
    }
            ...
            while ($currentRow++ < $highestRow) {
                ...
                if ($writeCurrentRow) {
                    ...
                    if ($rowDimension->getRowHeight() >= 0) {
                        ...
                        $objWriter->writeAttribute('ht', \PHPExcel\Shared\StringClass::FormatNumber($rowDimension->getRowHeight()));
                    }
                ...
                switch (strtolower($mappedType)) {
                    case 'inlinestr':    // Inline string
                        if (! $cellValue instanceof \PHPExcel\RichText) {
                            $objWriter->writeElement('t', \PHPExcel\Shared\StringClass::ControlCharacterPHP2OOXML(htmlspecialchars($cellValue)));
...
//                                $calculatedValue = $pCell->getCalculatedValue();
                                if (!is_array($calculatedValue) && substr($calculatedValue, 0, 1) != '#') {
                                    $objWriter->writeElement('v', \PHPExcel\Shared\StringClass::FormatNumber($calculatedValue));
                                } else {
