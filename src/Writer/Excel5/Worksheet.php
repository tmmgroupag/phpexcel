...
/**
 * \PHPExcel\Writer\Excel5\Worksheet
 *
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
            ...
            if ($cVal instanceof \PHPExcel\RichText) {
                ...
                $arrcRun = array();
                $str_len = \PHPExcel\Shared\StringClass::CountCharacters($cVal->getPlainText(), 'UTF-8');
                $str_pos = 0;
                ...
                foreach ($elements as $element) {
                    ...
                    // Position FROM
                    $str_pos += \PHPExcel\Shared\StringClass::CountCharacters($element->getText(), 'UTF-8');
                }
    ...
    private function writeRichTextString($row, $col, $str, $xfIndex, $arrcRun)
    {
        ...
        $str = \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeShort($str, $arrcRun);
    ...
    private function writeLabelSst($row, $col, $str, $xfIndex)
    {
        ...
        $str = \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($str);
    ...
    private function writeStringRecord($stringValue)
    {
        ...
        $data = \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($stringValue);
    ...
    public function writeUrlInternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        // character count
        $url_len = \PHPExcel\Shared\StringClass::CountCharacters($url);
        $url_len = pack('V', $url_len);

        $url = \PHPExcel\Shared\StringClass::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');
    ...
    private function writeSelection()
    {
        ...
        $selectedCells = \PHPExcel\Cell::splitRange($this->phpSheet->getSelectedCells());
        ...
        list($colFirst, $rwFirst) = \PHPExcel\Cell::coordinateFromString($first);
        $colFirst = \PHPExcel\Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
        --$rwFirst; // base 0 row index

        list($colLast, $rwLast) = \PHPExcel\Cell::coordinateFromString($last);
        $colLast = \PHPExcel\Cell::columnIndexFromString($colLast) - 1; // base 0 column index
    ...
    private function writeRangeProtection()
    {
        foreach ($this->phpSheet->getProtectedCells() as $range => $password) {
            ...
            $recordData .= \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
    ...
    private function writeHeader()
    {
        ...
        */
        ...
        $recordData = \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($this->phpSheet->getHeaderFooter()->getOddHeader());
        $length = strlen($recordData);
    ...
    private function writeFooter()
    {
        ...
        */
        ...
        $recordData = \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($this->phpSheet->getHeaderFooter()->getOddFooter());
        $length = strlen($recordData);
     ...
     * Set Escher object
     *
     * @param \PHPExcel\Shared\Escher $pValue
     */
    public function setEscher(\PHPExcel\Shared\Escher $pValue = null)
    {
        $this->escher = $pValue;
    }
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach ($dataValidationCollection as $cellCoordinate => $dataValidation) {
                ...
                $data .= \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($promptTitle);

                // error title
                $errorTitle = $dataValidation->getErrorTitle() !== '' ?
                    $dataValidation->getErrorTitle() : chr(0);
                $data .= \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($errorTitle);

                // prompt text
                $prompt = $dataValidation->getPrompt() !== '' ?
                    $dataValidation->getPrompt() : chr(0);
                $data .= \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($prompt);

                // error text
                $error = $dataValidation->getError() !== '' ?
                    $dataValidation->getError() : chr(0);
                $data .= \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($error);
    ...
    private function writeCFRule(\PHPExcel\Style\Conditional $conditional)
    {
        ...
        if ($bFormatFont == 1) {
            ...
            if ($conditional->getStyle()->getFont()->getName() == null) {
                ...
                $dataBlockFont = \PHPExcel\Shared\StringClass::UTF8toBIFF8UnicodeLong($conditional->getStyle()->getFont()->getName());
            }
            // Font Size
